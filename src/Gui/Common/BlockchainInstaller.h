...
private:
  ...
  const QDir m_atomiumDir;
  const QDir m_applicationDir;

  bool checkIfCurrentBlockchainExists() const;
  bool checkIfAtomiumBlockchainExists() const;
  bool getGenesisBlockFromBlockchain(char** _genesisBlockData) const;
  bool checkIfBlockchainOutdated(quint64& _current, quint64& _atomium) const;
  QFileInfo currentBlockchainInfo() const;
  QFileInfo atomiumBlockchainInfo() const;
