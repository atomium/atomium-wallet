...
BlockchainInstaller::BlockchainInstaller(QObject* _parent) : QObject(_parent), m_blockIndexesFileName("blockindexes.dat"), m_blocksFileName("blocks.dat"),
  m_atomiumDir(Settings::instance().getDataDir().absolutePath()), m_applicationDir(QDir::current()) {
}
...
void BlockchainInstaller::exec() {
  ...
  if (!checkIfAtomiumBlockchainExists()) {
    installBlockchain();
    return;
  }

  quint64 currentHeight;
  quint64 atomiumHeight;
  if (!checkIfBlockchainOutdated(currentHeight, atomiumHeight)) {
    return;
  }
  ...
  QuestionDialog dlg(tr("Blockchain installation"), QString(questionStringTemplate).arg(atomiumHeight).arg(currentHeight), nullptr);
...
bool BlockchainInstaller::checkIfAtomiumBlockchainExists() const {
  return m_atomiumDir.exists() && m_atomiumDir.exists(m_blocksFileName);
}
...
bool BlockchainInstaller::checkIfBlockchainOutdated(quint64& _sourceHeight, quint64& _targetHeight) const {
  ...
  QFile targetBlockIndexesFile(m_atomiumDir.absoluteFilePath(m_blockIndexesFileName));
...
QFileInfo BlockchainInstaller::atomiumBlockchainInfo() const {
  return QFileInfo(m_atomiumDir.absoluteFilePath(m_blocksFileName));
}
...
void BlockchainInstaller::installBlockchain() {
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blockIndexesFileName),
    m_atomiumDir.absoluteFilePath(m_blockIndexesFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blocksFileName), m_atomiumDir.absoluteFilePath(m_blocksFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
