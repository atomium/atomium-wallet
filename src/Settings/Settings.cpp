...
//
// Atomium is free software: you can redistribute it and/or modify
// it under the terms of the GNU Lesser General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Atomium is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU Lesser General Public License for more details.
//
// You should have received a copy of the GNU Lesser General Public License
// along with Atomium.  If not, see <http://www.gnu.org/licenses/>.
...
const char DEFAULT_WALLET_FILE_NAME[] = "atomium.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "pool.atomiumproject.com:3333";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("atomiumwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("atomiumwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("atomiumwallet.desktop");
  ...
  res = autorunSettings.contains("AtomiumWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("AtomiumWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.atomium.atomiumwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=Atomium Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("AtomiumWallet", appPath);
    } else {
      autorunSettings.remove("AtomiumWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\atomium", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:atomium");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\atomium\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\atomium\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("atomiumwallet.cfg"));
